...
export function StatCard({ icon, label, value, hint, trend, accent = 'navy', size = 'lg', className }: Props) {
  return (
    <Card className={cn('p-5', className)}>
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="text-xs font-medium text-ink-500 uppercase tracking-wider">{label}</p>
          <p className={cn('mt-2 font-bold text-ink-900 truncate', sizeClasses[size])}>{value}</p>
